...
export class AlibabaPlatform extends BasePlatform {
  ...
  async getProductList(params: any): Promise<ProductInfo[]> {
    try {
      ...
      const url = `https://www.alibaba.com/trade/search?fsb=y&IndexArea=product_en&CatId=&SearchText=${encodeURIComponent(category)}&page=${page}`;
  ...
  async getProductDetail(id: string): Promise<ProductInfo> {
    try {
      const url = `https://www.alibaba.com/product-detail/${id}.html`;
      const response = await this.crawler.crawl(url);
  ...
  async searchProducts(keyword: string): Promise<ProductInfo[]> {
    try {
      const url = `https://www.alibaba.com/trade/search?fsb=y&IndexArea=product_en&SearchText=${encodeURIComponent(keyword)}`;
      const response = await this.crawler.crawl(url);
      ...
      const products = await this.page.$$eval('.product-item', items => {
        return items.map(item => {
          ...
          return {
            ...
            images: [image],
            url: url.startsWith('http') ? url : `https://www.alibaba.com${url}`,
            specifications: {},
            seller: {},
            description: '',
            platform: 'alibaba',
